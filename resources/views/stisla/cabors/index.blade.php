      ...
      <div class="col-12">
        ...
        @if ($data->count() > 0)
          {{-- @if ($canExport) --}}
            <div class="card">
              <div class="card-header">
                <h4><i class="fa fa-fa-solid fa-grid-2-plus"></i> {!! __('Aksi Ekspor <small>(Server Side)</small>') !!}</h4>
                <div class="card-header-action">
                  ...
                  <tbody>
                    @foreach ($data as $item)
                      <tr>
                        ...
                        <td>
                          @if ($canUpdate)
                            @include('stisla.includes.forms.buttons.btn-edit', ['link' => route('cabors.edit', [$item->id])])
                          @endif
                          @if ($canDelete)
                            @include('stisla.includes.forms.buttons.btn-delete', ['link' => route('cabors.destroy', [$item->id])])
                          @endif
                        </td>
                      </tr>
                    @endforeach
        ...
        @else
          @include('stisla.includes.others.empty-state', ['title' => 'Data ' . $title, 'icon' => 'fa-solid fa-grid-2-plus', 'link' => $routeCreate])
        @endif
      </div>
