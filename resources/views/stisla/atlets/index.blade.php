      ...
      <div class="col-12">
        ...
        @if ($data->count() > 0)
          @if ($canExport)
            <div class="card">
              <div class="card-header">
                <h4><i class="fa fa-fa-solid fa-person"></i> {!! __('Aksi Ekspor <small>(Server Side)</small>') !!}</h4>
                <div class="card-header-action">
                  ...
                  <tbody>
                    @foreach ($data as $item)
                      <tr>
                        ...
                        <td>{{ $item->nama }}</td>
                        <td>
                            @foreach ($cabor->where('id', $item->cabor) as $cbr)
                                {{$cbr->nama}}
                            @endforeach
                        </td>
                        <td>{{ $item->nomor }}</td>
                        ...
                        <td>
                          @if ($canUpdate)
                            @include('stisla.includes.forms.buttons.btn-edit', ['link' => route('atlets.edit', [$item->id])])
                          @endif
                          @if ($canDelete)
                            @include('stisla.includes.forms.buttons.btn-delete', ['link' => route('atlets.destroy', [$item->id])])
                          @endif
                          <a href="{{route('atlets.show', $item->id)}}" class="btn btn-sm btn-success">
